...
export default function SearchPage() {
  ...
  useEffect(() => {
    ...
    setResults(list.filter(c => c.name.toLowerCase().includes(q) || (c.description || "").toLowerCase().includes(q)));
  }, [query, filter, all]);
  ...
  return (
    <div>
      <div style={{ marginBottom:20 }}>
        <h2 style={{ fontFamily:"Syne,sans-serif", fontSize:20, fontWeight:700, marginBottom:4 }}>Semantic Search</h2>
        <p style={{ fontSize:13, color:"#94A3B8" }}>Search your knowledge base.</p>
      </div>
      <div style={{ position:"relative", marginBottom:14 }}>
        <span style={{ position:"absolute", left:14, top:"50%", transform:"translateY(-50%)", fontSize:17, color:"#475569" }}>🔍</span>
        <input value={query} onChange={e => setQuery(e.target.value)} placeholder="What did I learn about Deadlock?"
          style={{ width:"100%", padding:"13px 16px 13px 44px", background:"#1A1C24", border:"1px solid rgba(255,255,255,0.13)", borderRadius:12, color:"#F1F5F9", fontSize:14, fontFamily:"DM Sans,sans-serif", outline:"none" }} />
      </div>
      <div style={{ display:"flex", gap:8, marginBottom:20, flexWrap:"wrap" }}>
        {["all","DSA","OS","DBMS","General"].map(f => (
          <button key={f} onClick={() => setFilter(f)} style={{ padding:"6px 14px", borderRadius:20, fontSize:12, border:"none", cursor:"pointer", fontFamily:"DM Sans,sans-serif", background:filter===f?"#6C63FF":"#1A1C24", color:filter===f?"#fff":"#94A3B8" }}>
            {f === "all" ? "All Topics" : f}
          </button>
        ))}
      </div>
      {!query.trim()
        ? <div style={{ textAlign:"center", padding:"60px 20px", color:"#475569" }}>Start typing to search</div>
        : results.length === 0
        ...
        : results.map(c => (
          <div key={c.id} onClick={() => revise(c.id)} style={{ padding:16, background:"rgba(26,28,36,0.95)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:10, marginBottom:8, cursor:"pointer" }}>
            ...
            <div style={{ fontSize:12, color:"#94A3B8", lineHeight:1.6 }}>{c.description}</div>
